...
const StatusPage = () => {
    ...
    const websites = [
        {
            name: 'NMS-Pro-File',
            url: 'https://nms.profile.co.th/login',
            usernameSelector: 'input[name="username"]',
            passwordSelector: 'input[name="password"]',
            buttonSelector: 'button.btn.btn-primary.btn-block',
            userInfoSelector: 'nav ul.nav.navbar-nav.navbar-right li.dropdown span.hidden-sm small',
        },
        {
            name: 'IT Assets',
        ...
        {
            name: 'Pro-File API (Admin app)',
            url: 'https://app.profile.co.th/admin/user/login',
            usernameSelector: 'input[name="login-form[login]"]',
            passwordSelector: 'input[name="login-form[password]"]',
            buttonSelector: 'button.btn.btn-primary.btn-block',
            userInfoSelector: 'div.user-panel div.pull-left.info p',
    ...
    const [results_SQL, setResults_SQL] = useState([]);
            ...
            <div className="w-full bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg mb-6">
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        {results_SQL.map((result, index) => (
                            <div
                                key={index}
                                className={`p-6 rounded-lg shadow-lg ${result.status === "ออนไลน์ปกติ" ? "bg-green-100" : "bg-red-100"
                                    }`}
                            >
                                ...
                                <p className="text-gray-700 dark:text-gray-300">
                                    สถานะ:{" "}
                                    <span
                                        className={`font-bold ${result.status === "ออนไลน์ปกติ"
                                                ? "text-green-600"
                                                : "text-red-600"
                                            }`}
                                    >
